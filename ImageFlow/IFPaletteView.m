...
- (BOOL)selectPreviousTreeTemplate;
{
  ...
  for (IFTemplateLayer* layer in self.visibleTemplateLayers) {
    if (layer.treeNode == cursorNode) {
      [cursors setTree:prev.tree node:prev.treeNode index:0];
...
- (BOOL)selectNextTreeTemplate;
{
  ...
  for (IFTemplateLayer* layer in self.visibleTemplateLayers) {
    if (selectNext) {
      [cursors setTree:layer.tree node:layer.treeNode index:0];
      [self updateCursorLayers];
      return YES;
  ...
  if (selectNext) {
    ...
    [cursors setTree:firstLayer.tree node:firstLayer.treeNode index:0];
  }
  return selectNext;
}
...
- (void)moveToNodeRepresentedBy:(IFTemplateLayer*)layer;
{
  [cursors setTree:layer.tree node:layer.treeNode index:0]; // FIXME: use correct index
